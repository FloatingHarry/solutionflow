...
import type { Account, ActivityList, BusinessCaseWorkspace, DeploymentWorkspace, DiscoveryWorkspace, Workflow } from "@/lib/types";
...
export default async function AccountPage({ params }: { params: Promise<{ accountId: string }> }) {
  ...
  try {
    [account, workflow, activities, discovery, businessCase, deployment] = await Promise.all([
    ...
    <div className="page accountPage">
      ...
      <header className="accountHeader">
        <div className="accountIdentity">
          ...
          <div>
            <span className="eyebrow">Account workspace {account.is_demo ? "· Synthetic demo" : ""}</span>
            <h1>{account.name}</h1>
